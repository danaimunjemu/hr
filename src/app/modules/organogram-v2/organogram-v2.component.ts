...
import { OrganogramV2Node, OrganogramV2Service } from './organogram-v2.service';
...
export class OrganogramV2Component implements AfterViewInit, OnDestroy {
  ...
  private readonly companyThemeCache = new Map<string, {
    accent: string;
    badgeBg: string;
    badgeBorder: string;
    badgeText: string;
  ...
  private readonly companyThemes = [
    { accent: '#2563eb', badgeBg: '#eff6ff', badgeBorder: '#bfdbfe', badgeText: '#1e3a8a' },
  ...
  loadDepartmentOrganogram(): void {
    const unitId = Number(this.unitIdInput || 0);
    if (!unitId) {
  ...
  private renderChart(): void {
    ...
    chartAny
      ...
      .linkUpdate(function (this: SVGPathElement) {
        this.setAttribute('stroke', '#6b7280');
  ...
  private nodeTemplate(node: { data: OrganogramV2Node; width: number; height: number }): string {
    ...
    const company = this.escapeHtml(data.companyName || 'N/A');
    const badge = this.escapeHtml(`${title} @ ${company}`);
    const initials = this.escapeHtml(data.initials || 'NA');
    const imageUrl = String(data.imageUrl || '').trim();
    const avatar = imageUrl
      ? `<img class="org-card-avatar-image" src="${this.escapeHtml(imageUrl)}" alt="${name}" />`
      : `<span class="org-card-avatar-initials">${initials}</span>`;
    const theme = this.getCompanyTheme(company);
    const classes = [
      'org-card',
      ...
      .filter(Boolean)
      .join(' ');
    const companyStyle = this.colorByOrganisation
      ? `--company-accent:${theme.accent};--company-badge-bg:${theme.badgeBg};--company-badge-border:${theme.badgeBorder};--company-badge-text:${theme.badgeText};`
      : '';
    ...
    return `
      <div class="${classes}" style="width:${node.width}px;height:${node.height}px;${companyStyle}">
        ...
        <div class="org-card-badge" title="${badge}">${badge}</div>
        <div class="org-card-dept" title="${department}">${department}</div>
      </div>
    `;
  }
