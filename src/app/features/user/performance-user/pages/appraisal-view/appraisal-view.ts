...
@Component({
  ...
  standalone: false,
  templateUrl: './appraisal-view.html',
  styleUrls: ['./appraisal-view.scss']
})
export class AppraisalViewComponent implements OnInit {
    ...
    this.appraisalService.getById(id)
      ...
      .subscribe({
        ...
        error: (err: any) => {
          ...
          this.router.navigate(['/app/performance-user/appraisals']);
        }
      });
  }
  ...
  saveItem(item: AppraisalItem): void {
    ...
    this.appraisalService.selfRate({
      ...
      .subscribe({
        next: () => {
          this.message.success('Self-rating saved successfully');
          this.editingItemId.set(null);
          this.loadAppraisal(appraisalData.id);
        },
        error: () => {
          this.message.error('Failed to save self-rating');
        }
      });
  }
  ...
  canSubmitAppraisal(): boolean {
    const appraisalData = this.appraisal();
    if (!appraisalData || appraisalData.status !== 'DRAFT') return false;

    // Check if all items have been rated
    return appraisalData.appraisalItems.every(item =>
      item.selfRating !== null && item.selfComment !== null
    );
  }
  ...
  submitAppraisal(): void {
    ...
    this.appraisalService.submitAppraisal(appraisalData.id)
